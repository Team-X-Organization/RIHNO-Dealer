...
// interceptKafkaDial rewrites localhost to my_rihno_kafka before connecting
func interceptKafkaDial(ctx context.Context, network, addr string) (net.Conn, error) {
	host, port, err := net.SplitHostPort(addr)
	if err == nil {
		if host == "localhost" || host == "::1" || host == "127.0.0.1" {
			addr = "my_rihno_kafka:" + port
		}
	}
	var d net.Dialer
...
func sendToKafka(payload *MetricsPayload) error {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		writeErr := kafkaWriter.WriteMessages(ctx, msg)
		cancel()

		if writeErr == nil {
			log.Printf("[Kafka] Successfully sent metrics for %s (key: %s, attempt: %d)", payload.AgentName, messageKey, attempt)
			return nil
		}

		if attempt < maxRetries {
			log.Printf("[Kafka] Write attempt %d/%d failed for %s: %v — retrying in %s",
				attempt, maxRetries, payload.AgentName, writeErr, backoff)
			time.Sleep(backoff)
			backoff *= 2
		} else {
...
func main() {
	...
	kafkaBrokers := []string{"my_rihno_kafka:9092"} // Use Docker container name for inter-container communication
